...
def constrained_clustering_optimized(customer_df, min_size=180, max_size=190, max_radius=20):
...
def constrained_clustering_with_radius(customer_df, min_size=180, max_size=200, max_radius=300):
    """
...
def assign_proximity_customers_to_existing_portfolios(unassigned_customers_df, customer_assignments, branch_df, proximity_threshold=20, max_portfolio_size=200):
    """
...
def create_centralized_clusters_with_radius_and_assign(unassigned_customers_df, branch_df, 
                                                     used_branches, min_size=180, max_size=200, max_radius=300):
    """
...
def create_final_centralized_portfolios(unassigned_customers_df, branch_df, used_branches, max_size=200):
    """
    ...
    while True:
        ...
        for outlier in outlier_customers:
            ...
            for idx in nearest_indices:
                ...
                # Check if target portfolio has capacity (updated to 200)
                if portfolio_sizes.get(target_au, 0) < 200:
...
def enhanced_customer_au_assignment_with_unique_branches(customer_df, branch_df):
    ...
    Updated with new portfolio size constraints: min_size=180, max_size=200, initial=180-190
    """
    ...
    # Step 1: Create first INMARKET clusters (20-mile radius, max_size=190)
    ...
    clustered_customers, cluster_info = constrained_clustering_optimized(
        customer_df, min_size=180, max_size=190, max_radius=20
    )
    ...
    if unassigned_customer_indices and inmarket_results:
        ...
        proximity_results, unassigned_after_proximity, updated_customer_assignments = assign_proximity_customers_to_existing_portfolios(
            ...
            proximity_threshold=20, max_portfolio_size=200
        )
    ...
    if unassigned_after_proximity:
        ...
        clustered_customers_2, cluster_info_2 = constrained_clustering_optimized(
            remaining_customers_df, min_size=180, max_size=190, max_radius=40
        )
    ...
    if unassigned_after_second_inmarket:
        ...
        centralized_results, remaining_after_centralized, used_branches = create_centralized_clusters_with_radius_and_assign(
            remaining_unassigned_df, branch_df, used_branches, min_size=180, max_size=200, max_radius=300
        )
    ...
    if remaining_after_centralized:
        ...
        final_centralized_results, used_branches = create_final_centralized_portfolios(
            final_unassigned_df, branch_df, used_branches, max_size=200
        )
    ...
    print(f"Portfolio Size Constraints: Min=180, Max=200, Initial=180-190")
    ...
    if len(result_df) > 0:
        ...
        print(f"Portfolios with 180-200 customers: {len(portfolio_sizes[(portfolio_sizes >= 180) & (portfolio_sizes <= 200)])}")
...
# Max portfolio size: 200 customers
# Initial cluster size: 180-190 customers  
