...
def constrained_clustering_optimized(customer_df, min_size=180, max_size=200, max_radius=20):
...
def constrained_clustering_with_radius(customer_df, min_size=180, max_size=210, max_radius=300):
    """
...
def create_centralized_clusters_with_radius_and_assign(unassigned_customers_df, branch_df, 
                                                     used_branches, min_size=180, max_size=210, max_radius=300):
    """
    ...
    while True:
        ...
        for outlier in outlier_customers:
            ...
            for idx in nearest_indices:
                ...
                # Check if target portfolio has capacity (updated to 210)
                ...
                # Check for trade opportunity (updated to 210)
...
def rebalance_portfolio_sizes(result_df, branch_df, min_size=180, search_radius=50):
    """
...
def fill_undersized_portfolios_from_unassigned(result_df, unassigned_customer_indices, customer_df, branch_df, min_size=180, max_radius=40):
    """
...
def enhanced_customer_au_assignment_with_unique_branches(customer_df, branch_df):
    ...
    Updated with new portfolio size constraints: min_size=180, max_inmarket_size=210
    """
    ...
    # Step 1: Create first INMARKET clusters (20-mile radius, max_size=200)
    ...
    clustered_customers, cluster_info = constrained_clustering_optimized(
        customer_df, min_size=180, max_size=200, max_radius=20
    )
    ...
    if unassigned_after_proximity:
        ...
        clustered_customers_2, cluster_info_2 = constrained_clustering_optimized(
            remaining_customers_df, min_size=180, max_size=200, max_radius=40
        )
    ...
    # Step 7: Balance INMARKET portfolios to minimum size (180)
    if len(result_df) > 0:
        print("Step 7: Balancing INMARKET portfolios...")
        result_df = rebalance_portfolio_sizes(result_df, branch_df, min_size=180)
    ...
    if len(result_df) > 0 and unassigned_after_second_inmarket:
        ...
        result_df, unassigned_after_second_inmarket = fill_undersized_portfolios_from_unassigned(
            result_df, unassigned_after_second_inmarket, customer_df, branch_df, min_size=180, max_radius=40
        )
    ...
    if unassigned_after_second_inmarket:
        ...
        centralized_results, remaining_after_centralized, used_branches = create_centralized_clusters_with_radius_and_assign(
            remaining_unassigned_df, branch_df, used_branches, min_size=180, max_size=210, max_radius=300
        )
    ...
    print(f"Portfolio Size Constraints: Min=180, Max INMARKET=210")
    ...
    if len(result_df) > 0:
        ...
        print(f"\nPortfolio Size Distribution:")
        print(f"Portfolios with < 180 customers: {len(portfolio_sizes[portfolio_sizes < 180])}")
        print(f"Portfolios with 180-210 customers: {len(portfolio_sizes[(portfolio_sizes >= 180) & (portfolio_sizes <= 210)])}")
        ...
        print(f"Median distance to assigned AU: {result_df['DISTANCE_TO_AU'].median():.2f} miles")
    
    return result_df

# Usage example:
# Enhanced assignments with updated portfolio size constraints
# Min portfolio size: 180 customers
# Max in-market portfolio size: 210 customers  
