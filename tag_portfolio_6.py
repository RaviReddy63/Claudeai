...
def extract_au_500_portfolio(customer_au_assignments):
    """
    Extract AU 500 portfolio from customer_au_assignments and return filtered dataframe and AU 500 data
    
    Parameters:
    customer_au_assignments: DataFrame with customer AU assignments
    
    Returns:
    tuple: (filtered_df_without_au_500, au_500_portfolio_df)
    """
    print("=== EXTRACTING AU 500 PORTFOLIO ===")
    
    # Create a copy to avoid modifying the original
    customer_au_assignments_copy = customer_au_assignments.copy()
    
    # Filter for AU 500 portfolio
    au_500_portfolio = customer_au_assignments_copy[
        customer_au_assignments_copy['ASSIGNED_AU'] == 500
    ].copy()
    
    # Remove AU 500 from the main dataframe
    filtered_assignments = customer_au_assignments_copy[
        customer_au_assignments_copy['ASSIGNED_AU'] != 500
    ].copy()
    
    print(f"AU 500 portfolio customers: {len(au_500_portfolio)}")
    print(f"Remaining customers after removing AU 500: {len(filtered_assignments)}")
    
    if len(au_500_portfolio) > 0:
        # Get portfolio type and customer count for AU 500
        au_500_types = au_500_portfolio['TYPE'].unique()
        print(f"AU 500 portfolio types: {au_500_types}")
        
        for portfolio_type in au_500_types:
            type_count = len(au_500_portfolio[au_500_portfolio['TYPE'] == portfolio_type])
            print(f"  {portfolio_type}: {type_count} customers")
    
    return filtered_assignments, au_500_portfolio

def tag_au_500_to_joshua_nguyen(au_500_portfolio_df, client_groups_df, tagging_results):
    """
    Tag AU 500 portfolio to JOSHUA NGUYEN and combine with existing tagging results
    
    Parameters:
    au_500_portfolio_df: DataFrame with AU 500 portfolio data
    client_groups_df: DataFrame with client groups data for financial metrics
    tagging_results: DataFrame with existing tagging results
    
    Returns:
    DataFrame: Combined tagging results including AU 500 portfolio
    """
    print("=== TAGGING AU 500 PORTFOLIO TO JOSHUA NGUYEN ===")
    
    if len(au_500_portfolio_df) == 0:
        print("No AU 500 portfolio data to tag")
        return tagging_results
    
    # Get unique portfolio types for AU 500
    au_500_summary = au_500_portfolio_df.groupby('TYPE').agg({
        'CG_ECN': 'count'
    }).reset_index()
    au_500_summary.columns = ['TYPE', 'CUSTOMER_COUNT']
    
    joshua_tags = []
    
    # Process each portfolio type for AU 500
    for _, portfolio_type_row in au_500_summary.iterrows():
        portfolio_type = portfolio_type_row['TYPE']
        customer_count = portfolio_type_row['CUSTOMER_COUNT']
        
        # Get customers for this portfolio type
        customers_for_type = au_500_portfolio_df[
            au_500_portfolio_df['TYPE'] == portfolio_type
        ]['CG_ECN'].dropna().tolist()
        
        # Calculate financial metrics for AU 500 portfolio
        new_avg_deposit, new_avg_gross_sales, new_avg_bank_revenue = get_portfolio_financial_metrics(
            customers_for_type, client_groups_df
        )
        
        # Create tagging entry for AU 500
        joshua_tags.append({
            'NEW_AU': 500,
            'NEW_TYPE': portfolio_type,
            'NEW_CUSTOMER_COUNT': customer_count,
            'TAGGED_TO_PORTFOLIO': '',  # No specific portfolio assignment
            'TAGGED_TO_EMPLOYEE': '',  # No specific employee assignment
            'TAGGED_TO_MANAGER': 'JOSHUA NGUYEN',
            'TAGGED_TO_DIRECTOR': '',  # No director assignment specified
            'TAGGED_TO_AU': '',
            'TAGGING_CRITERIA': 'MANUAL_ASSIGNMENT_JOSHUA_NGUYEN',
            'DISTANCE_MILES': None,
            'CUSTOMER_OVERLAP_COUNT': 0,
            'EXISTING_PORTFOLIO_SIZE': 0,
            'EXISTING_AVG_DEPOSIT_BAL': None,
        ...
        print(f"Tagged AU 500 {portfolio_type} portfolio to JOSHUA NGUYEN:")
        print(f"  Customer Count: {customer_count}")
        print(f"  Avg Deposit: {new_avg_deposit}")
        print(f"  Avg Gross Sales: {new_avg_gross_sales}")
        print(f"  Avg Bank Revenue: {new_avg_bank_revenue}")
    
    # Convert to DataFrame
    joshua_tagging_df = pd.DataFrame(joshua_tags)
    
    # Combine with existing tagging results
    if len(tagging_results) > 0:
        combined_results = pd.concat([tagging_results, joshua_tagging_df], ignore_index=True)
    else:
        combined_results = joshua_tagging_df
    
    print(f"\nCombined results:")
    print(f"  Original tagging results: {len(tagging_results)}")
    print(f"  AU 500 (JOSHUA NGUYEN) results: {len(joshua_tagging_df)}")
    print(f"  Total combined results: {len(combined_results)}")
    
    return combined_results

def tag_new_portfolios_to_mojgan_portfolios_with_au_500(customer_au_assignments, active_portfolio_df, client_groups_df, branch_df):
    """
    Complete portfolio tagging process including AU 500 assignment to JOSHUA NGUYEN
    
    This function combines the main tagging process with AU 500 special handling.
    To disable AU 500 special handling, simply call tag_new_portfolios_to_mojgan_portfolios directly.
    
    Parameters:
    customer_au_assignments: DataFrame with customer AU assignments
    active_portfolio_df: DataFrame with active portfolio data
    client_groups_df: DataFrame with client groups data
    branch_df: DataFrame with branch data
    
    Returns:
    DataFrame: Complete tagging results including AU 500 assignment
    """
    print("=== COMPLETE PORTFOLIO TAGGING WITH AU 500 PROCESSING ===")
    
    # Step 1: Extract AU 500 portfolio
    filtered_assignments, au_500_portfolio = extract_au_500_portfolio(customer_au_assignments)
    
    # Step 2: Run main tagging process with filtered data (without AU 500)
    ...
    # Step 3: Tag AU 500 to JOSHUA NGUYEN and combine results
    tagging_results = tag_au_500_to_joshua_nguyen(
        au_500_portfolio, 
        client_groups_df, 
        tagging_results
    )
    ...
    print(f"\n=== FINAL SUMMARY ===")
    print(f"Total portfolios processed: {len(tagging_results)}")
    if len(tagging_results) > 0:
        joshua_portfolios = tagging_results[tagging_results['TAGGED_TO_MANAGER'] == 'JOSHUA NGUYEN']
        mojgan_portfolios = tagging_results[tagging_results['TAGGED_TO_DIRECTOR'] == 'MOJGAN MADADI']
        print(f"Portfolios assigned to JOSHUA NGUYEN: {len(joshua_portfolios)}")
        print(f"Portfolios tagged to MOJGAN MADADI: {len(mojgan_portfolios)}")
    
    return tagging_results

# Usage Options:

# Option 1: Complete process with AU 500 handling
# tagging_results = tag_new_portfolios_to_mojgan_portfolios_with_au_500(customer_au_assignments, ACTIVE_PORTFOLIO, CLIENT_GROUPS_DF_NEW, branch_df)

# Option 2: Main process only (without AU 500 special handling)
