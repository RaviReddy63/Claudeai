...
def create_final_centralized_portfolios(unassigned_customers_df, branch_df, used_branches, max_size=210):
    """
    ...
    while True:
        ...
        for outlier in outlier_customers:
            ...
            for idx in nearest_indices:
                ...
                # Check if target portfolio has capacity (max 210)
                if portfolio_sizes.get(target_au, 0) < 210:
                ...
                elif portfolio_sizes.get(target_au, 0) == 210:
...
def rebalance_portfolio_sizes(result_df, branch_df, min_size=160, search_radius=50):
    """
...
def fill_undersized_portfolios_from_unassigned(result_df, unassigned_customer_indices, customer_df, branch_df, min_size=160, max_radius=40):
    """
...
def enhanced_customer_au_assignment_with_unique_branches(customer_df, branch_df, au_list=None):
    """
    Enhanced main function ensuring each AU is assigned to only ONE cluster/portfolio
    Includes Step 6 optimization with updated size parameters (160-180 initial, 210 final)
    
    NEW: Step 0 - Create In-Market portfolios for predefined AUs first (if au_list provided)
    """
    ...
    # Step 0: Create In-Market portfolios for predefined AUs (NEW STEP)
    predefined_results = []
    remaining_customer_indices = list(customer_df.index)
    
    if au_list and len(au_list) > 0:
        predefined_results, remaining_customer_indices = assign_customers_to_predefined_aus(
            customer_df, branch_df, au_list, proximity_radius=20, max_portfolio_size=210
        )
        
        # Mark predefined AUs as used
        for au in au_list:
            if any(result['ASSIGNED_AU'] == au for result in predefined_results):
                used_branches.add(au)
        
        print(f"Marked {len(used_branches)} predefined AUs as used")
    
    # Get remaining customers for subsequent clustering steps
    remaining_customer_df = customer_df.loc[remaining_customer_indices] if remaining_customer_indices else pd.DataFrame()
    
    # Step 1: Create first INMARKET clusters (20-mile radius, 160-180 size) - on remaining customers
    print("Step 1: Creating first INMARKET clusters (20-mile radius, 160-180 customers)...")
    inmarket_results = []
    unassigned_customer_indices = []
    
    if not remaining_customer_df.empty:
        clustered_customers, cluster_info = constrained_clustering_optimized(
            remaining_customer_df, min_size=160, max_size=180, max_radius=20
        )
        ...
        if len(cluster_info) > 0:
            ...
            for branch_au, customers in customer_assignments.items():
                ...
                for customer in customers:
                    ...
                    customer_data = remaining_customer_df.loc[customer_idx]
            ...
            unassigned_customer_indices = remaining_customer_indices.copy()
    else:
        unassigned_customer_indices = []
    
    print(f"Total unassigned customers after first INMARKET: {len(unassigned_customer_indices)}")
    
    # Step 4: Check proximity of unassigned customers to identified AUs (max 210)
    proximity_results = []
    unassigned_after_proximity = unassigned_customer_indices.copy()
    
    # Combine predefined and clustered results for proximity check
    all_current_results = predefined_results + inmarket_results
    
    if unassigned_customer_indices and all_current_results:
        # Create customer_assignments from all current results for proximity check
        customer_assignments = {}
        for result in all_current_results:
            ...
            if not matching_customers.empty:
                customer_idx = matching_customers.index[0]
        ...
        proximity_results, unassigned_after_proximity, updated_customer_assignments = assign_proximity_customers_to_existing_portfolios(
            ...
            proximity_threshold=20, max_portfolio_size=210
        )
    
    print(f"Total unassigned customers after proximity check: {len(unassigned_after_proximity)}")
    
    # Step 5: Create second INMARKET clusters (40-mile radius, 160-180 size)
    print("Step 5: Creating second INMARKET clusters (40-mile radius, 160-180 customers)...")
    second_inmarket_results = []
    unassigned_after_second_inmarket = unassigned_after_proximity.copy()
    
    if unassigned_after_proximity:
        remaining_customers_df_2 = customer_df.loc[unassigned_after_proximity]
        ...
        clustered_customers_2, cluster_info_2 = constrained_clustering_optimized(
            remaining_customers_df_2, min_size=160, max_size=180, max_radius=40
        )
        ...
        if len(cluster_info_2) > 0:
            ...
            for branch_au, customers in customer_assignments_2.items():
                ...
                for customer in customers:
                    customer_idx = customer['customer_idx']
                    customer_data = remaining_customers_df_2.loc[customer_idx]
    ...
    all_results = predefined_results + inmarket_results + proximity_results + second_inmarket_results
    ...
    # Step 7: Balance INMARKET portfolios to minimum size (160)
    if len(result_df) > 0:
        print("Step 7: Balancing INMARKET portfolios...")
        result_df = rebalance_portfolio_sizes(result_df, branch_df, min_size=160)
    ...
    if len(result_df) > 0 and unassigned_after_second_inmarket:
        ...
        result_df, unassigned_after_second_inmarket = fill_undersized_portfolios_from_unassigned(
            result_df, unassigned_after_second_inmarket, customer_df, branch_df, min_size=160, max_radius=40
        )
    
    # Step 9: Create CENTRALIZED clusters (160-200 size)
    centralized_results = []
    ...
    if unassigned_after_second_inmarket:
        ...
        centralized_results, remaining_after_centralized, used_branches = create_centralized_clusters_with_radius_and_assign(
            remaining_unassigned_df, branch_df, used_branches, min_size=160, max_size=200, max_radius=300
        )
    
    # Step 10: Handle final unassigned customers as centralized portfolios (max 210)
    final_centralized_results = []
    ...
    if remaining_after_centralized:
        ...
        final_centralized_results, used_branches = create_final_centralized_portfolios(
            final_unassigned_df, branch_df, used_branches, max_size=210
        )
    ...
    # Print summary
    print(f"\n=== FINAL SUMMARY WITH PREDEFINED AUs ===")
    if au_list and len(au_list) > 0:
        print(f"Predefined AUs processed: {len(au_list)}")
        predefined_customers = len(predefined_results)
        print(f"Customers assigned to predefined AUs: {predefined_customers}")
    print(f"Initial portfolio size: 160-180 customers")
    print(f"Final maximum portfolio size: 210 customers")
    ...
    if len(result_df) > 0:
        ...
        print(portfolio_summary.head(10))  # Show first 10 portfolios
        
        # Portfolio size distribution
        portfolio_sizes = result_df['ASSIGNED_AU'].value_counts()
        print(f"\nPortfolio Size Statistics:")
        print(f"Minimum portfolio size: {portfolio_sizes.min()}")
        print(f"Maximum portfolio size: {portfolio_sizes.max()}")
        print(f"Average portfolio size: {portfolio_sizes.mean():.1f}")
        print(f"Portfolios with < 160 customers: {len(portfolio_sizes[portfolio_sizes < 160])}")
        print(f"Portfolios with > 210 customers: {len(portfolio_sizes[portfolio_sizes > 210])}")
...
# Usage examples:
# Enhanced assignments with predefined AUs
# au_list = [101, 102, 103]  # Your predefined AU list
# assignments = enhanced_customer_au_assignment_with_unique_branches(customer_df, branch_df, au_list)
# assignments.to_csv('customer_au_assignments_with_predefined_aus.csv', index=False)

# Without predefined AUs (original behavior):
# assignments = enhanced_customer_au_assignment_with_unique_branches(customer_df, branch_df)
# assignments.to_csv('customer_au_assignments_updated_sizes.csv', index=False)import pandas as pd
import numpy as np
...
def assign_customers_to_predefined_aus(customer_df, branch_df, au_list, proximity_radius=20, max_portfolio_size=210):
    """
    Create In-Market portfolios for predefined AUs by assigning customers within proximity_radius
    Returns assigned customers and remaining unassigned customers
    """
    if not au_list or len(au_list) == 0:
        print("No predefined AUs provided, skipping predefined AU assignment step")
        return [], list(customer_df.index)
    
    print(f"Step 0: Creating In-Market portfolios for {len(au_list)} predefined AUs within {proximity_radius} miles...")
    
    # Filter branch_df to only include the predefined AUs
    predefined_branches = branch_df[branch_df['BRANCH_AU'].isin(au_list)].copy()
    
    if len(predefined_branches) == 0:
        print("Warning: None of the predefined AUs found in branch_df")
        return [], list(customer_df.index)
    
    print(f"Found {len(predefined_branches)} predefined AUs in branch data")
    
    # Get customer and branch coordinates
    customer_coords = customer_df[['LAT_NUM', 'LON_NUM']].values.astype(np.float64)
    branch_coords = predefined_branches[['BRANCH_LAT_NUM', 'BRANCH_LON_NUM']].values.astype(np.float64)
    
    # Calculate distance matrix between all customers and predefined AUs
    distance_matrix = compute_distance_matrix(customer_coords, branch_coords)
    
    predefined_results = []
    assigned_customer_indices = set()
    
    # Track portfolio sizes to respect max_portfolio_size
    portfolio_sizes = {au: 0 for au in au_list}
    
    # For each AU, find customers within proximity and assign closest ones first
    for i, (_, branch_row) in enumerate(predefined_branches.iterrows()):
        au = branch_row['BRANCH_AU']
        
        # Get distances from all customers to this AU
        distances_to_au = distance_matrix[:, i]
        
        # Find customers within proximity_radius
        within_radius_mask = distances_to_au <= proximity_radius
        
        if not np.any(within_radius_mask):
            print(f"No customers found within {proximity_radius} miles of AU {au}")
            continue
        
        # Get candidate customers with their distances
        candidates = []
        for j, (customer_idx, customer_data) in enumerate(customer_df.iterrows()):
            if within_radius_mask[j] and customer_idx not in assigned_customer_indices:
                candidates.append({
                    'customer_idx': customer_idx,
                    'distance': distances_to_au[j],
                    'customer_data': customer_data
                })
        ...
        # Assign customers up to max_portfolio_size
        assigned_to_this_au = 0
        for candidate in candidates:
            if assigned_to_this_au >= max_portfolio_size:
                break
            ...
            assigned_to_this_au += 1
            portfolio_sizes[au] += 1
        
        print(f"Assigned {assigned_to_this_au} customers to predefined AU {au}")
    
    # Get remaining unassigned customers
    unassigned_customer_indices = [idx for idx in customer_df.index if idx not in assigned_customer_indices]
    
    print(f"Predefined AU assignment complete:")
    print(f"  - Total customers assigned: {len(assigned_customer_indices)}")
    print(f"  - Remaining unassigned customers: {len(unassigned_customer_indices)}")
    
    # Print portfolio sizes for predefined AUs
    for au in au_list:
        if portfolio_sizes[au] > 0:
            print(f"  - AU {au}: {portfolio_sizes[au]} customers")
    
    return predefined_results, unassigned_customer_indices

def constrained_clustering_optimized(customer_df, min_size=160, max_size=180, max_radius=20):
...
def constrained_clustering_with_radius(customer_df, min_size=160, max_size=200, max_radius=300):
    """
...
def assign_proximity_customers_to_existing_portfolios(unassigned_customers_df, customer_assignments, branch_df, proximity_threshold=20, max_portfolio_size=210):
    """
...
def create_centralized_clusters_with_radius_and_assign(unassigned_customers_df, branch_df, 
                                                     used_branches, min_size=160, max_size=200, max_radius=300):
    """
